...
FILE =          'experiments/resnet_scriptable.py'

# Your HPC username
USERNAME =      'njanwani'

# num epochs to train
EPOCHS =        {cond : 15 for cond in CONDITIONS}

# Downsampling of train data
DOWNSAMPLE =    {cond : 0.2 for cond in CONDITIONS}
DOWNSAMPLE['Pleural Effusion'] = 0.1
DOWNSAMPLE['No Finding'] = 0.1

# Model to run
MODEL =         {cond : 152 for cond in CONDITIONS}
...
cd {DIRECTORY}
python -u {FILE + f' -c "{cond}" -r {MODEL[cond]} -e {EPOCHS[cond]} -d {DOWNSAMPLE[cond]} -b {BATCH_SIZE[cond]} -s {IMAGE_RESIZE[cond]}'}
            '''
        )
